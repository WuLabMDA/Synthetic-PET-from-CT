    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            if total_iters % opt.display_freq == 0:   # display images on visdom and save images to a HTML file
                ...
                #print(model.get_current_visuals()['fake_B'].shape)#torch.Size([5, 3, 512, 512])
                #print(model.get_current_visuals()['real_A'].shape)#torch.Size([5, 7, 512, 512])
                #print(model.get_current_visuals()['real_B'].shape)#torch.Size([5, 3, 512, 512])
            ...
            # I added losses here for Tensorboard
            losses = model.get_current_losses()
            loss_list.append(  [  losses['G_GAN'],   losses['G_L1'],    losses['D_real'],    losses['D_fake']  ]  )# I added
            iter_data_time = time.time()

        #I added Tensorboard  Are the losses computed on an instance or being averaged?
        visuals = model.get_current_visuals()     #visuals['real_A'][i,2,:,:], visuals['fake_B'].shape, visuals['real_B']
        ...
        writer.add_images('CT',       torch.unsqueeze(visuals['real_A'][:,mid_rA_idx,:,:], dim=1), epoch) #torch.unsqueeze(visuals['real_A'][:,2,:,:], dim=1)
        ...
        print(visuals['fake_B'].shape)#torch.Size([5, 3, 512, 512])
