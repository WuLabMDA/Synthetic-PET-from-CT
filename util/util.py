...
def nifti_to_5chl(path, file, start_slide = 0, end_slide = -1):
    #path = 'E:/data/CTtoPET/pix2pix_5_1/testA/'
    #file = 'CTPET_3D__000000.nii'
    #nifti_to_5chl(path, file)
    CT = io.imread(os.path.join(path, file), plugin='simpleitk')
    print(CT.shape)
    count = 0
    for k in range(start_slide, CT.shape[0]):  # Removing the first and last images as they are not reliable
        #print(k)
        CT_k = np.array( CT[k,:,:] )
...
# To be completed:
def slides_to_nifti(path):
    import nibabel as nib

    file_list = np.sort([file for file in os.listdir(path)])
    if file_path.endswith=='.nii.gz':
        whole_img = nib.load(file)
        PET = np.zeros(whole_img.get_fdata().shape)

    for i, file_path in enumerate( file_list ):
        
        #load the Nifti to only extract the affine function
        if file_path.endswith != '.nii.gz':
            PET[:,:,i] = np.load(file_path)
            #PET[:,:,i] =np.rot90( np.flipud( np.load(file_path)), -1) 
